...
import {
  transcribeWords, buildAssCaptions, saveAss, buildOverlayAss, buildKeywordAss, buildHookAss,
} from "./transcribe.mjs";
import { assFilter } from "./fonts.mjs";
import {
  colorGrade, manualColor, smoothFilter, voiceCleanFilter, voicePitchTempo,
...
export async function interiorEdit(videoPath, opts = {}) {
  ...
  const segments = tr ? (tr.segments || []).map((s) => ({ start: s.start, end: s.end, text: s.text })) : [];
  return { outPath, meta, transcriptText, segments, sourceVideo: videoPath, voicePath: voicePath || null };
}

// Áp text đã sửa tay: thay .word của từng câu theo tỉ lệ, giữ mốc thời gian câu.
// editedSegments: mảng chuỗi (mỗi dòng 1 câu). Ghép lại theo thứ tự segment hiện có.
function applyEditedText(tr, editedSegments) {
