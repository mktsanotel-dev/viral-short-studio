...
export async function interiorEdit(videoPath, opts = {}) {
  const {
    ...
    muteVideo = false,          // 🔇 tắt tiếng GỐC của video (không đụng giọng đọc nếu có)
    ...
    normalize = true,
    musicPath = null, musicVol = 0.14,   // 🎵 nhạc nền (tự lặp, hạ âm lượng, trộn dưới giọng)
  ...
  // ---------- Render cuối: chữ + logo + audio (giọng / tiếng gốc) + NHẠC NỀN ----------
  onLog("→ Render cuối (chữ + logo + nhạc nền)...");
  const args = ["-hide_banner", "-y", "-i", workVideo];   // input 0 = video
  let nIdx = 0, voiceIdx = null, musicIdx = null, logoIdx = null;
  if (audioFile) { args.push("-i", audioFile); voiceIdx = ++nIdx; }
  const hasMusic = !!(musicPath && fs.existsSync(musicPath));
  if (hasMusic) { args.push("-i", musicPath); musicIdx = ++nIdx; }
  ...
  if (logoPath && fs.existsSync(logoPath)) {
    args.push("-loop", "1", "-i", logoPath); logoIdx = ++nIdx;
  ...
  // ----- Audio: giọng đọc > tiếng GỐC (nếu KHÔNG tắt & video CÓ tiếng) > không; trộn NHẠC NỀN nếu có -----
  // muteVideo chỉ bỏ ÂM GỐC của video; giọng đọc riêng (nếu có) vẫn giữ nguyên.
  const silent = muteVideo && !audioFile;
  const origHasAudio = audioFile ? false : (!silent && (await probe(workVideo).catch(() => ({}))).hasAudio);
  const mainMap = audioFile ? `${voiceIdx}:a` : (origHasAudio ? "0:a" : null);
  const mainLabel = mainMap ? `[${mainMap}]` : null;
  const addA = (s) => { complex += `${complex ? ";" : ""}${s}`; };
  let aOut = null;
  if (hasMusic) {
    addA(`[${musicIdx}:a]volume=${musicVol},aloop=loop=-1:size=2e9,aformat=channel_layouts=stereo:sample_rates=48000[mus]`);
    if (mainLabel) { addA(`${mainLabel}[mus]amix=inputs=2:normalize=0:duration=first[amx]`); aOut = "[amx]"; }
    else aOut = "[mus]";                       // tắt tiếng gốc + có nhạc → chỉ còn nhạc nền
    if (normalize) { addA(`${aOut}loudnorm=I=-14:TP=-1.5:LRA=11[aout]`); aOut = "[aout]"; }
  } else if (mainLabel) {
    if (normalize) { addA(`${mainLabel}loudnorm=I=-14:TP=-1.5:LRA=11[aout]`); aOut = "[aout]"; }
    else aOut = mainMap;                        // map trực tiếp (không qua filter)
  }
  if (silent && !hasMusic) onLog("  🔇 Tắt âm thanh gốc của video");
  if (hasMusic) onLog(`  🎵 Nhạc nền: ${path.basename(musicPath)} (âm lượng ${Math.round(musicVol * 100)}%)`);

  const mapArgs = [];
  if (complex) mapArgs.push("-filter_complex", complex, "-map", vOut);
  else mapArgs.push("-map", "0:v");
  if (aOut) mapArgs.push("-map", aOut);

  const audioArgs = aOut ? ["-c:a", "aac", "-b:a", "192k", "-ar", "48000", "-ac", "2"] : ["-an"];
  await run(FFMPEG, [
    ...args, ...mapArgs,
    "-r", String(FPS), ...encoder(useGpu), "-pix_fmt", "yuv420p",
    ...audioArgs, "-shortest", "-movflags", "+faststart", outPath,
  ], { cwd: WORK, onLog: (l) => onLog("  " + l) });
