...
export async function textToSpeech(text, {
  ...
  // edge-tts gọi dịch vụ ONLINE nên thỉnh thoảng rớt mạng → TỰ ĐỘNG THỬ LẠI.
  const MAX_TRY = 3;
  let lastErr = null;
  try {
    for (let attempt = 1; attempt <= MAX_TRY; attempt++) {
      ...
      if (lastErr && attempt < MAX_TRY) {
        onLog(`  ⚠ lần ${attempt} lỗi (mạng chập chờn?) → thử lại…`);
  ...
  if (lastErr) {
    throw new Error(
      `Tạo giọng AI lỗi sau ${MAX_TRY} lần thử (edge-tts cần internet ổn định). Chi tiết: ` +
      String(lastErr.message).replace(/\s+/g, " ").slice(0, 200)
    );
  }
