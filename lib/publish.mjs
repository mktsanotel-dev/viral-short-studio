...
export async function makeThumbFor({ videoPath, title = "Video", thumbPhotoDir = null, thumbName = BRAND.name, id = "thumb", atSec = null, onLog = () => {} }) {
...
export async function publishOutputs(items = [], opts = {}) {
  const {
    ...
    thumbPhotoDir = null, thumbName = BRAND.name, loai = "Video", onLog = () => {},
  } = opts;
  const out = [];
    ...
    if (makeThumb && !it.thumbPath && it.outPath) {
      ...
      try {
        it.thumbPath = await makeThumbFor({
          ...
          thumbPhotoDir, thumbName, id: `pub-${i}`, onLog,
        });
