...
Style: Pop,${fontName},${fontSize},${primary},${highlight},${outline},&H64000000,-1,0,0,0,100,100,0,0,1,7,3,2,60,60,${marginV},1
...
`;
  ...
  for (const cue of cues) {
    ...
    if (style === "karaoke") {
      // Highlight lần lượt từng từ bằng \k (theo centisecond).
      let text = "";
      for (const w of cue) {
        const durCs = Math.max(1, Math.round((w.end - w.start) * 100));
        text += `{\\k${durCs}}${esc(w.word)} `;
      }
      // Hiệu ứng nảy nhẹ khi xuất hiện.
      const anim = `{\\fad(90,60)\\t(0,120,\\fscx112\\fscy112)\\t(120,220,\\fscx100\\fscy100)}`;
      lines.push(`Dialogue: 0,${toAssTime(start)},${toAssTime(end)},Pop,,0,0,0,,${anim}${text.trim()}`);
    } else {
      const text = cue.map((w) => esc(w.word)).join(" ");
      const anim = `{\\fad(80,60)\\t(0,120,\\fscx110\\fscy110)\\t(120,220,\\fscx100\\fscy100)}`;
...
// Rút danh sách câu {start,end,text} từ transcript — để giao diện cho SỬA PHỤ ĐỀ.
export function transcriptSegments(tr) {
  if (!tr) return [];
  return (tr.segments || []).map((s) => ({ start: s.start, end: s.end, text: s.text }));
}
...
export function buildHookAss(text, {
  ...
  fontName = "Roboto", fontSize = 104, marginV = 170,
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Hook,${fontName},${fontSize},${primary},${primary},${outline},${back},-1,0,0,0,100,100,0,0,3,6,4,8,70,70,${marginV},1
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Ovl,${fontName},${fontSize},${primary},${primary},${outline},${back},-1,0,0,0,100,100,0,0,1,6,3,${align},70,70,${marginV},1
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Kw,${fontName},${fontSize},${primary},${primary},${outline},${back},-1,0,0,0,100,100,0,0,1,7,4,5,60,60,0,1
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Thumb,${fontName},${fontSize},${primary},${primary},${outline},${back},-1,0,0,0,100,100,0,0,3,7,5,2,60,60,220,1
