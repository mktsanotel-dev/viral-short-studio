...
async function buildMontage(scenes, D, { id, W, H, sceneDur = 3.6, colorPreset, useGpu, onLog }) {
  ...
  // 🎬 Dãy cảnh phủ >= D giây. CHỐNG LẶP triệt để:
  //  • mỗi VÒNG duyệt hết file theo THỨ TỰ NGẪU NHIÊN MỚI (không còn chu kỳ cố định lặp lại),
  //  • KHÔNG để 1 file xuất hiện 2 lần LIÊN TIẾP (kể cả nối giữa 2 vòng),
  //  • mỗi cảnh ĐỘ DÀI dao động ±25% cho nhịp linh động,
  //  • kết hợp lấy ĐOẠN NGẪU NHIÊN giữa clip (bên dưới).
  const rshuffle = (arr) => { const a = [...arr]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; };
  const seq = [];   // [{file, kind, dur}]
  let total = 0, lastFile = null, guard = 0;
  while (total < D && guard++ < 400 && seq.length < 120) {
    const round = rshuffle(scenes);
    if (lastFile && round.length > 1 && round[0].file === lastFile) [round[0], round[1]] = [round[1], round[0]];
    for (const s of round) {
      if (total >= D || seq.length >= 120) break;
      const dur = +(sceneDur * (0.8 + Math.random() * 0.5)).toFixed(2);   // 0.8–1.3× sceneDur
      seq.push({ ...s, dur }); total += dur; lastFile = s.file;
    }
  }
  const N = seq.length;
  onLog(`→ Dựng montage: ${N} cảnh (từ ${scenes.length} file), xoay NGẪU NHIÊN không lặp liền → phủ ${D.toFixed(1)}s`);
  if (scenes.length <= 3) onLog(`  ⚠ chỉ ${scenes.length} file cảnh → nên bỏ thêm nhiều clip vào thư mục để bớt trùng.`);

  // Thời lượng từng video (probe 1 lần/file) → lấy ĐOẠN NGẪU NHIÊN GIỮA video, KHÔNG luôn từ đầu.
  const durCache = new Map();
  for (const s of seq) {
    if (s.kind === "video" && !durCache.has(s.file)) {
      let d = 0; try { d = (await probe(s.file)).duration || 0; } catch { d = 0; }
      durCache.set(s.file, d);
    }
  }
  ...
  seq.forEach((s) => {
    if (s.kind === "image") {
      args.push("-loop", "1", "-t", String(s.dur), "-i", s.file);
    } else {
      // Điểm bắt đầu NGẪU NHIÊN trong khoảng còn đủ độ dài cảnh → mỗi lần 1 cảnh khác, kể cả cùng 1 file.
      const vdur = durCache.get(s.file) || 0;
      const maxStart = Math.max(0, vdur - s.dur - 0.1);
      const start = maxStart > 0.2 ? +(Math.random() * maxStart).toFixed(2) : 0;
      if (start > 0) randomized++;
      args.push("-ss", String(start), "-stream_loop", "-1", "-t", String(s.dur), "-i", s.file);
    }
  });
  if (randomized) onLog(`  🎲 ${randomized}/${seq.length} cảnh cắt từ đoạn ngẫu nhiên giữa video`);
  let fc = "";
  seq.forEach((_, i) => {
